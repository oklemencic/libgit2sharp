...
namespace LibGit2Sharp.Tests
{
    public class FilterBranchFixture : BaseFixture
    {
        ...
        {
            var commits = repo.Commits.QueryBy(new Filter { Since = repo.Refs }).ToArray();
            repo.Refs.RewriteHistory(commits, commitHeaderRewriter: c =>
        ...
        {
            var origRefs = repo.Refs.Where(x => !x.IsTag()).ToArray();
            ...
            Assert.Throws<Exception>(
                ...
                repo.Refs.RewriteHistory(
                    ...
                    directReferenceRewriter: (r, t) =>
                        {
                            if (r.CanonicalName == "refs/heads/master")
                                throw new Exception("BREAK");
                            return repo.Refs.DefaultDirectReferenceRewriter(r, t);
                        }));

            var newRefs = repo.Refs.Where(x => !x.IsTag()).Where(x => !x.CanonicalName.StartsWith("refs/original/")).ToArray();
            Assert.Equal(newRefs,
                         origRefs);
        }

        [Fact]
        public void RefRewritingRollsBackOnFailureEvenWithTypeChanges()
        {
            repo.Refs.Add("refs/heads/asymbolic", repo.Refs["refs/heads/master"]);
            repo.Refs.Add("refs/heads/zsymbolic", repo.Refs["refs/heads/master"]);
            var origRefs = repo.Refs.Where(x => !x.IsTag()).ToArray();
            ...
            Assert.Throws<Exception>(
                ...
                repo.Refs.RewriteHistory(
                    ...
                    directReferenceRewriter: (r, t) =>
                        {
                            if (r.CanonicalName == "refs/heads/br2")
                                return repo.Refs.Add("refs/heads/br2", repo.Refs["refs/heads/packed-test"], true);
                            return repo.Refs.DefaultDirectReferenceRewriter(r, t);
                        },
                    symbolicReferenceRewriter: (r, t) =>
                        {
                            if (r.CanonicalName == "refs/heads/asymbolic")
                                return repo.Refs.Add("refs/heads/asymbolic", r.ResolveToDirectReference().Target.Id, true);
                            if (r.CanonicalName == "refs/heads/zsymbolic")
                                throw new Exception("BREAK");
                            return repo.Refs.DefaultSymbolicReferenceRewriter(r, t);
                        }));

            Assert.Equal(repo.Refs.Where(x => !x.IsTag())
                             .Where(x => !x.CanonicalName.StartsWith("refs/original/")).ToArray(),
                         origRefs);
        }
        ...
        public void CanPreventRefsFromBeingBackedUp()
        {
            ...
            repo.Refs.RewriteHistory(repo.Head.Commits, c => CommitRewriteInfo.From(c, message: "abc"),
                directReferenceRewriter: (r, t) => repo.Refs.UpdateTarget(r, t, "filter-branch: rewrite"));

            Assert.Equal(numberOfRefs, repo.Refs.Count());
        }

        [Fact]
        public void RewritesDirectReferencesFirst()
        {
            bool hasSeenASymbolicRef = false;
            repo.Refs.Add("refs/heads/asymbolic", repo.Refs["refs/heads/master"]);
            var commits = repo.Commits.QueryBy(new Filter { Since = repo.Refs }).ToArray();
            repo.Refs.RewriteHistory(commits,
                                     directReferenceRewriter: (r, t) =>
                                         {
                                             Assert.False(hasSeenASymbolicRef);
                                             return r;
                                         },
                                     symbolicReferenceRewriter: (r, t) =>
                                         {
                                             hasSeenASymbolicRef = true;
                                             return r;
                                         });
            Assert.True(hasSeenASymbolicRef);
        }
